<template>
  <view class="write">
    ...
    <view class="content">
      <!-- content为多个html代码块，可以分别操作 -->
      <view v-for="(item,index) in content" :ref="'content'+index" :class="{'active':active===index}" :key="index"
      ...
      <view class="ad-bottom" @click="showAdListModel()">
        ...
        <!-- 名片广告 -->
        <view class="advertise_display_card" v-else-if="adContent['advertisementType']==='BUSINESS_CARD_ADVERTISEMENT'"
          :style="{backgroundImage:'url('+adContent.backdropPic+')'}">
          <view class="name-box">
    ...
    <view class="model-wrap" :class="{'model-wrap-show':showAdList}" v-if="showAdList">
      <view class="model">
        ...
        <view class="model-type-list">
          <view class="type-view" v-for="(val,key) in adObj" :key="key" :class="{'active':advertisementType === key}"
        ...
        <scroll-view class="model-content" scroll-y="true" @scrolltolower="scrollAdtolower">
          ...
          <view v-else-if="adObj[advertisementType]==='名片'">
            <view class="advertise_display_card" v-for="(ad,index) in adList" :key="index" @click="selectAdList(ad)"
...
export default {
  ...
  methods: {
    ...
    reEdit(seqId, offerRewardSeqId) {
      this.$api.get('/o2oMyArticle/getArticleInfo', {
        ...
        if (res.code === 200) {
          ...
          let content = [];
          // section为我们提交时在每个html文本块添加的标识，以section标签开头的内容即为内部文章
          if (articleContent.startsWith('<section>')) { // 内部文章
            // <section>1111</section><section>222</section><section>3333</section>
            // 去掉首尾得到1111</section><section>222</section><section>3333，再以</section><section>分割既能得到每一个代码块
            ...
            let virdom = new Parser(articleContent, this).parse(); // 获取外部文章解析的虚拟dom对象
            content = this.articleToList(virdom, content); //把虚拟dom对象转化为多个可操作html代码块的数组
          }
          // 如果有广告id，提取广告
    ...
    selectEdit(index) {
      ...
      this.contentType = (contentTypeArr[0] && contentTypeArr[0][1]) || ''; // 选中的段落类型
      this.active = index;
    },
    // 删除内容
    deleteContent() {
    ...
    uploadVideo() {
      ...
      uni.chooseVideo({
        ...
        success: video => {
          ...
          uni.uploadFile({
            url: this.api + '/sys/appeal/videoUpload',
            ...
            success: (uploadFileRes) => {
              ...
              if (res.code === 200) {
                ...
                self.content.splice(self.active + 1, 0, `<video src="${res.data.viewUrl}" controls style="border-radius:20rpx;width:100%;height:400rpx;object-fit:contain;"></video>`);
                self.active = -1;
              } else {
                uni.showToast({
                  icon: 'none',
